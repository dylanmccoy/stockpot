...
- `/code-review` (Standards + Spec): Spec axis clean. Standards axis found no hard violations — six judgement-call smells, all inherited verbatim from the merged `provision.py` / `restore.py` siblings. Actioned the two slice-local ones: dropped a redundant `int()` cast on `rowcount`; wrapped a few >88-char lines in `test_recover_cli.py`. Deliberately skipped: extracting the shared `make_engine` + `sqlite:///` pre-check + `dispose()` plumbing into a helper (would edit two already-reviewed modules — belongs in its own cleanup chore, not this slice); moving `_schema_db` / `_login` / `_run` test helpers to `conftest.py` (the `test_provision*` pair duplicates them the same way); `database_url` appearing in `RecoverError` text (spec scope is local `sqlite:///`, no secret; consistent with `provision.py`); `raw` name and the `"users"`-only schema probe (both match `provision.py`).
